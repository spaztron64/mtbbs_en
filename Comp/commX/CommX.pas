//**********************************************************************//
//                                                                      //
//  ～ ＲＳ２３２Ｃ通信コンポーネント Ver 1.06 ～                       //
//                                                                      //
//          動作環境：  Windows 95/98/NT/2000                           //
//        使用ｺﾝﾊﾟｲﾗ：  Delphi 5.0 (Build 6.18)                         //
//                                                                      //
//        ファイル名：  CommX.pas                                       //
//                                                                      //
//          制作者名：  エックス (KYY06770)                             //
...
//      最終更新日付：  2000/06/27                                      //
//                                                                      //
//  ※ 送信と受信にスレッドとメッセージを使用し、バックグラウンドで     //
//   処理を行いながら効率よく実行できるようになっている。               //
//                                                                      //
//**********************************************************************//
unit CommX;
...
TCommTransThread = class(TThread)
  private
    FOwner      : TComponent;             // 親コンポーネント
    CommHandle  : THandle;                // 通信用ファイルハンドル
    CommWindow  : THandle;                // ウィンドウのハンドル
    procedure HalfCtrlStart;
    procedure HalfCtrlEnd;
  protected
    procedure Execute; override;
  public
    MsgClearFlg : Boolean;                // メッセージ・クリアフラグ
    HalfWaitTim : Integer;                // 半二重制御時の制御時間
...
TCommReceiveThread = class(TThread)
  ...
  protected
    procedure Execute; override;
  public
    ExitHandle  : THandle;                // イベント待ち終了シグナル
...
private
  ...
  FOnReceive      : TCommReceiveEvent;    // 受信通知イベント
  FOnBreak        : TCommBreakEvent;      // ブレーク信号イベント
  FOnError        : TCommErrEvent;        // 通信エラーイベント

  TransThread     : TCommTransThread;     // 送信用スレッド
  ReceiveThread   : TCommReceiveThread;   // 受信用スレッド
