...
var
  ...
  RasGetConnectStatus : TRasGetConnectStatus;
  RasGetErrorString : TRasGetErrorString;
  RasHangUp : TRasHangUp;
  RasGetEntryDialParams : TRasGetEntryDialParams;
  RasApiFlag : Word; // 0:Loading OK, 1:Not Find RASAPI32, 2:Abnormal RASAPI32
...
procedure DllReadError(sz:AnsiString);
begin
//  ShowMessage(sz + ' is not find');
  RasApiFlag := 1;  //1:Not Find RASAPI32
end;

procedure FuncReadError(sz:AnsiString);
begin
//  ShowMessage(sz + ' is abnormal');
  RasApiFlag := 2;  //2:Abnormal RASAPI32
end;

initialization
   RasApiFlag := 0;
   DllHandle := LoadLibrary(RAS_DLL);
   if DllHandle = 0 then
      begin
         DllReadError(RAS_DLL);
         Exit;
      end;
   Ras_func := 'RasDialA';
   RasDial:= TRasDial(GetProcAddress(DllHandle, Ras_func));
   if @RasDial = nil then FuncReadError(Ras_func);

   Ras_func := 'RasEnumConnectionsA';
   RasEnumConnections := TRasEnumConnections(GetProcAddress(DllHandle, Ras_func));
   if @RasEnumConnections = nil then FuncReadError(Ras_func);

   Ras_func := 'RasEnumEntriesA';
   RasEnumEntries := TRasEnumEntries(GetProcAddress(DllHandle, Ras_func));
   if @RasEnumEntries = nil then FuncReadError(Ras_func);

   Ras_func := 'RasEnumEntriesA';
   RasEnumEntries:= TRasEnumEntries(GetProcAddress(DllHandle, Ras_func));
   if @RasEnumEntries = nil then FuncReadError(Ras_func);

   Ras_func := 'RasGetConnectStatusA';
   RasGetConnectStatus:= TRasGetConnectStatus(GetProcAddress(DllHandle, Ras_func));
   if @RasGetConnectStatus = nil then FuncReadError(Ras_func);

   Ras_func := 'RasGetErrorStringA';
   RasGetErrorString:= TRasGetErrorString(GetProcAddress(DllHandle, Ras_func));
   if @RasGetErrorString = nil then FuncReadError(Ras_func);

   Ras_func := 'RasHangUpA';
   RasHangUp:= TRasHangUp(GetProcAddress(DllHandle, Ras_func));
   if @RasHangUp = nil then FuncReadError(Ras_func);
