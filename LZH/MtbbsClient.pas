...
(*******************************************
   MTBBS クライアント　コンポーネント
   Copyright (C) 1998.4 By Yoshihiro Myokan
 *******************************************)

{

 本コンポーネントは、TTCPClientクラスを使用しているため、
 WSockets.pasを別途入手＆インストールする必要があります。
   http://www.delphi32.com/ (URL自信なし^^;)

   http://memeber.nifty.ne.jp/myo/mtbbs/
   myo@tky2.3web.ne.jp
...
uses
  ...
  Wsockets_,Comm,MyoComm;


type

  TCommMode = (CM_Modem,CM_Telnet);
  THostKind = (HK_mtbbs,HK_ktbbs,HK_WWWBoard);
Const
  HostKindStrs : Array[THostKind] of string =
                  ('MTBBS','KTBBS','--------');
Type
  TAttribute = Record
       New          : Boolean;
  ...
  TMtOnReceiveChatRoomList = procedure (Sender: TObject;List : String) of object;
  ...
  private
     ...
     FOnReceiveChatRoomList   : TMtOnReceiveChatRoomList;
     FOnLogin                 : TMtOnLogin;
...
// メインスレッドと同期
    ...
    property OnReceiveChatRoomList : TMtOnReceiveChatRoomList read FOnReceiveChatRoomList
                                                    write FOnReceiveChatRoomList;
...
//    AutoDeleteMail : Boolean;
    ...
    Procedure OnReceiveChatRoomList;
...
Procedure TWorkThread.OnReceiveChatRoomList;
Begin
    _.FOnReceiveChatRoomList(_,String(P1^));
End;
          ...
          While (_.Rp = _.Wp) and (_.Status=CS_None) and Not Terminated do
           ...
           if _.Status=CS_None then
            Begin
             ...
             If Assigned(_.FOnReceiveChatRoomList) then
                If (a = '<< チャットルーム一覧 >>') then
                   ReadChatRoomList;
...
Procedure TWorkThread.ReadChatRoomList;
Var
      a,b : String;
Begin
  ...
  Sts('チャットルーム一覧を拾得中');
      ...
      b := '';
       ...
       While (a<>'') and Not Terminated do
         ...
         End;
        P1 := @b;
        Synchronize(OnReceiveChatRoomList);
  Sts('チャットルーム一覧一覧を拾得しました');
