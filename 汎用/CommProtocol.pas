{
 ...
 ---------------------------------------------------------------
  X,Y,ZModem,MLINKシリアルファイル転送コンポーネント   Ver 1.04
 ---------------------------------------------------------------
...
※TransSys.PasのM-LINK, ZMODEM部分に関しては、RTBBS(Resource版 TURBO-BBS)
 / Comserveさんのかかれたソースがもととなっているそうです。
  ...
  1997.9.13 Ver 1.04
     ...
     OnUpOrDownload　イベントを追加.
     オブジェクトインスペクタでProtocolプロパティをpr_Zmodemにしていても実行時に
     pr_Xmodemになってしまうバグを修正.

 現在分かっている不具合は1200BPSなど・・低速での送受信が非常に不安定である
 (原因不明)ということです。
...
Type
    ...
    TUpOrDownLoaded = procedure (Sender: TObject;Success: Boolean)
                        of object;
...
//                     YMODEM_G_BAT,
     ...
     TranModeType    = (TM_NONE,TM_UPLOAD,TM_DOWNLOAD);

 TMCommTran = class(TThread)
  private
  protected
  ...
  TMYOCOMM = class(TComm)
 private

     FProtocol       : prottype;
     FUpOrDownLoaded : TUpOrDownLoaded;

     Tran_Mode       : TranModeType;
     Success_        : Boolean;

     MCS            : TMyoCommStatus;   // ステータス
     ...
     procedure xmitblock(Var rpt ; len:integer);
  ...
  public
     ...
     Abort         : boolean;
     FOnCommReceiveMyo : TCommNotifyRxTxEvent;
     Procedure DownLoad(FileName:string);
     Procedure UpLoad  (FileName:string;SendFileName : String);

     constructor Create(AOwner :Tcomponent); override ;

     constructor Write(const Buffer; Size: Integer);
     constructor Read(Buffer: PChar; Size: Integer);
     constructor SendString(S : String);

     Procedure  CommReceiveMyo (Sender: TObject; Size: Word);

     function CTS : Boolean;
     function RLSD : Boolean;

     property TranMode      : TranModeType   read Tran_Mode;
     property Success       : Boolean        read Success_;
     property Status        : TMyoCommStatus read MCS;

  published

    property Protocol         : prottype read FProtocol write FProtocol default pr_ZModem;
    property OnUpOrDownLoaded : TUpOrDownLoaded read FUpOrDownLoaded write FUpOrDownLoaded;
...
procedure Register;
Const
   ProtocolStr : Array [ProtType] of String =(
                   'XMODEM [SUM] 128byte',
                   'XMODEM [CRC] 128byte',
                   'XMODEM [CRC] 1024byte',
                   'YMODEM',
...
implementation
 ...
 Var
        File_Recv_Buf        : array[0..50000] of char;
        InReady              : boolean;
        Rp: integer;
        Wp: integer;
        ...
        Xprotocol           : XprotType;
        Ybatch              : Boolean;
        OnMYOCommReceive    : TCommNotifyRxTxEvent;

        chksum		        :  byte;
        _crc16		        :  smallint;//word;
        _crc32		        :  longint;
        lastsent		:  byte;
        crcmode	                :   crctype;
        cancelled               : boolean;
...
//   転送用スレッド生成
constructor TMCommTran.Create(AOwner: TComponent);
var
  dwEvtMask: DWORD;
begin
...
Procedure TMCommTran.Execute;
Begin
  With TMyoComm(FOWner) do
   Begin
      FOnCommReceiveMyo := OnCommReceive;     // もとのonReceive先 アドレスを保存
      OnCommReceive     := CommReceivemyo;
      Success_          := True;
    Try
            GOption     := False;
      Case protocol of
...
{        YMODEM_BAT          :           Begin
    ...
    Except
      Success_:=False;
    End;
      OnCommReceive     := FOnCommReceiveMyo;
      Tran_Mode:=Tm_None;
      FUpOrDownLoaded(FOWner,Success_);
...
constructor TMYOCOMM.Create(AOwner :Tcomponent);
Begin
...
constructor TMYOCOMM.Write(Const Buffer; Size: Integer);
Begin
     inherited Write(Buffer,Size);
End;

constructor TMYOCOMM.Read(Buffer: PChar; Size: Integer);
Begin
    inherited Read(Buffer,Size);
End;

constructor TMYOCOMM.SendString(S :string);
Begin
    inherited SendString(s);
End;

Procedure TMYOCOMM.CommReceiveMyo(Sender: TObject; Size: Word);
Var
 i:Integer;
 Buf:Pchar;
 s:String;
begin
  ...
  Read(Buf,Size);
       ...
       For i:=0 to Size-1 do
        begin
         File_Recv_Buf[wp]:=buf[i];
         inc(wp);
         if wp>49999 then wp:=0;
        End;
        InReady:=True;
...
function TMYOCOMM.CTS: boolean;
Var
  ModemStat : DWORD;
begin
 if Handle>=0 then
  Begin
   GetCommModemStatus(handle,ModemStat);
   Result:=((ModemStat and MS_CTS_ON)=MS_CTS_ON);
  End
  Else
   Result:=False;
...
function TMYOCOMM.RLSD: boolean;
Var
  ModemStat : DWORD;
begin
 if Handle>=0 then
  Begin
   GetCommModemStatus(handle,ModemStat);
   Result:=((ModemStat and MS_RLSD_ON)=MS_RLSD_ON);
  End
  Else
   Result:=False;
...
Procedure TMyoComm.xmitblock(Var rpt ; len:integer);
(*シリアル出力ポートがレディならblockを出力する（ローカルモードなら出力しない）
  not cts なら何もせずに戻る *)
  begin
	if cts and (Len>0) then begin
                write(rpt,len);
	end;
...
Procedure TMYOCOMM.DownLoad(FileName : String);
Begin

  If  Tran_Mode<>TM_None then
   Begin
       Success_ := False;
       Exit;
   end;

  Tran_Mode:=Tm_DownLoad;
  MCS.FileName:=FileName;
  MCS.FileSize  :=9999999;
  MCS.SendFileName:='';
  ClearStatus;
  TMCommTran.Create(Self);

End;


Procedure TMYOCOMM.UpLoad(FileName,SendFileName : String);
Begin
  ...
  If SendFileName='' then MCS.SendFileName:=ExtractFileName(FileName)
                     else MCS.SendFileName:=SendFileName;

  MCS.FileName := FileName;
  MCS.FileSize := Get_File_Size;
  If MCS.FileSize=0 then
   Begin
       Success_:=False;
       exit;
   end;
...
procedure TMyoComm.ClearStatus;
Begin

    Abort:=False;
    MCS.StartTime   := GetTickCount;
    MCS.TranSize    := 0;
    MCS.ErrCount    := 0;
    MCS.GetFileName :=  '';
...
procedure Register;
begin
  RegisterComponents('Comm', [TMYOCOMM]);
end;

  procedure TMyoComm.calcCRC(data:byte);
	begin
	  case crcmode of
		sum  : inc(chksum,data);
		crc16:
  ...
  Procedure TMyoComm.calcblkCRC(var dt; n:word);
	type b = array [1..1024] of byte;
	var  i : word;
	begin
	  case crcmode of
		sum  :
		  for i:=1 to n do inc(chksum,b(dt)[i]);
		crc16:
		  for i:=1 to n do
			_crc16 := crc16table[Hi(_crc16) xor b(dt)[i]] xor
											(_crc16 shl 8);
		crc32:
		  for i:=1 to n do
  ...
  Function TMyoComm.takeCRC:longint;
	begin
	  case crcmode of
  ...
  Function TMyoComm.matchCRC:longint;
	begin
	  case crcmode of
		crc16:matchCRC:=$0000;
...
Function TMyoComm.time1970(PT:TDateTime): string;	(*1970.1.1からの秒数(Octal)*)
...
Function TMyoComm.GMTtoPACKT(GMT:LongInt) :TDateTime;(*1970.1.1からの秒数からPackTime*)
...
Function TMyoComm.alpha(filename:String): boolean;
  var strpos: integer;
	  dots:		byte;
  begin
	dots := 0;
...
Function TMyoComm.inbyte: byte;
  var temp: char;
  begin

	  while (not inready) and CTS do ;//Application.ProcessMessages;
	  temp := recvchar;
          Result := ord(temp);
  end;


Function TMyoComm.timedin: boolean;
  (* １秒以内に受信データがあれば真を返す。used in X/Y/ZMODEM *)
  var
	begcnt	: integer;
  begin
	begcnt:=GetTickCount;
	while cts and (not inready) and (GetTickCount-begcnt<1000) and not abort do
            ;
	timedin:=inready;
  end;


Procedure TMyoComm.acknak(var inch: byte; time: integer);
...
*)
  ...
  begin
	...
	repeat
	  ...
	  inc(loop);
	until (inch in [ACK, NAK, CAN, C, G, INI, ABT, ETR, EXT, MED]) or (loop >= loopend) or not cts or abort;
  end;


Function TMyoComm.acknakout(ch : byte): boolean;
...
//          writeln(comtest.D_F,inttostr(times)+chr(ch)+inttostr(Integer(inready)));
	  loops := 0;
	  xmitchar(chr(ch));
	  while (loops < 10) and not timedin and not abort do inc(loops);
	  inc(times);
	until inready or (times > 9) or not cts or abort;
...
Procedure TMyoComm.Fill_EOF_Code(var pt; n:word);
...
Procedure TMyoComm.sendcalc(ch : byte);
  begin
	xmitchar(chr(ch));
	calcCRC(ch);
  end;
...
Function TMyoComm.send1block(blkunit: integer; blocknum: SmallInt): SmallInt;
...
*)
  ...
  begin
	...
	until (inch = ACK) or (inch = CAN) or (tries = 10) or not cts or abort;
	if inch = ACK then send1block := 0
	else			   send1block := 9;
	//_^.CnStat := zfile;
  end;

Procedure TMyoComm.Xdownload( var successful:boolean);
  var
	  inch, loop	: byte;
	  blocknum		: integer;
	  temp			: line;
  ...
  begin
	...
	successful := true;
	  ...
	  if FileExists(MCS.filename) then begin

          	assignFile(f_,MCS.filename);
                reset(F_);
		done	 := false;
		rslt	 := 0;
		blocknum := 0;
		...
		repeat
		  if rslt = 0 then begin
			if eof(f_) then done := true
			  ...
			  blockreadm(filebuf_[0], blkunit);
                          MCS.TranSize:=MCS.TranSize+blkunit*128;
		  ...
		  if done then rslt := 1;
		until (inch = CAN) or (rslt <> 0) or not cts or abort;
		successful := (rslt = 1);
		tries := 0;
		if successful and cts then repeat
		  ...
		  acknak(inch, 60);
		  inc(tries);
		until (inch=ACK) or (tries > 10) or not cts or abort;
		if cts and (inch <> CAN) and not successful then
...
Procedure TMyoComm.clearbuff;
  begin
    FillChar(filebuf_[0], 128, chr(0));
  end;


Procedure TMyoComm.setheader;
  var tmp    : line;
  begin
    clearbuff;
    str(Get_File_Size, tmp);
    tmp := MCS.SendFileName+#0+tmp+' '+time1970(FileDateToDateTime(FileAge(MCS.FileName)));
    tmp := AnsiLowerCase(tmp);        {小文字化(MS漢字はｽﾙｰ)}
    Move(tmp[1], filebuf_[0], length(tmp));
  end;

Procedure TMyoComm.Ydownload(var successful:boolean);
  var
	  inch, loop	: byte;
	  blocknum		: integer;
	  temp			: line;
...
//			if hoststat = cn then write(cr + 'Sent #', blocknum:4);
		...
		if cts then xmitblock(cancan, sizeof(cancan));
		waiting(1000);
		successful := false;
	  end;

	While (not cts) and Abort do ;//application.processMessages;

	if cts and (Ybatch and successful) then begin
	  acknak(inch, 10);
	  clearbuff;
	  rslt := send1block(1, 0);
	end;
...
Procedure TMyoComm.Mdownload(var successful:boolean);
(* The resource版から書き直し HAL*)
  ...
  Procedure setMheader;
	...
	begin
	  s := MCS.SendFileName + ' ';
	  DT:=FileDateToDateTime(FileAge(MCS.filename));
  ...
  begin
    successful := true;
	  ...
	  if not FileExists(MCS.fileName) then
	    rslt := 8
	  else begin
		...
		acknak(inch, 60);
		if (not cts) or (inch <> ord(INI)) or abort then rslt := 9;
		if rslt = 0 then begin
		  setMheader;
		  acknak(inch, 10);
		  fsize := Get_File_Size;
		  if inch <> INI then rslt := 9
			...
			while cts and (fsize > 0) and (rslt = 0) and not abort do begin
			  ...
			  while (wk1 < 8) and (fsize > 0) do begin
				...
				xmitblock(filebuf_[wk1 shl 7], sbyte);
                                ...
                                MCS.TranSize:=MCS.TranSize+Sbyte;
				dec(fsize, sbyte);
				inc(wk1);
			  end;
			  inc(blocknum);
//			  if hoststat = cn then write(cr + 'M-LINK Sent #', blocknum:4);
			  if inready then begin
				inch := byte(recvchar);
				if (inch = ABT) or (inch = EXT) or Abort then rslt := 9;
			  end;
			end;
			if cts and (rslt = 0) and Not(Abort) then begin
			  acknak(inch, 10);
			  if (inch <> ETR) or Abort then
				rslt := 9
			  else begin
     //				_DL(fls)[i].rslt := true;
			  end;
	  ...
	  if not successful then begin
		...
		flowctrl(true);}
		waiting(10000);
		if cts then xmitchar(#$0d);
		waiting(1000);
//		if rslt = 8 then lineout('Can''t found ' + dir + _DL(fls)[i].lfnam);
	  end;

     	until (tries>10) or  Abort or successful or not cts;
        //Application.ProcessMessages;
	xmitchar(chr($0D));
	acknak(inch, 60);
	waiting(1000);
...
Function TMyoComm.recchar(var error: boolean): byte;
(*errorは１秒以内にデータがあれば偽*)
  var temp: byte;
  begin
	if not cts or abort then error := true
	else begin
...
Function TMyoComm.recv1block(var blkunit: integer; var blocknum: smallint): integer;
  ...
  begin
	...
	repeat
	  ...
	  if rslt = 2 then begin
		if (not g_option) and cts then begin
		  if not acknakout(NAK) then rslt := 9;
		end
		else rslt := 9;
	  end;
	  if tries > 6 then crcmode := CRC16;
	until (rslt=0) or (rslt=1) or (rslt=9) or not cts or abort;
	recv1block := rslt;
  end;
...
Procedure TMyoComm.Xupload( var successful:boolean);
  var
  ...
  begin
	...
	if cts and (rslt = 0) and (not acknakout(opening)) then rslt := 9;
...
//	if not successful then eraseM;
  ...
  Procedure TMyoComm.Yupload(var successful:boolean);
  var
	  blocknum, byteloc : smallint;
	  blkunit			: integer;
	  locblock			: integer;
...
Procedure TMyoComm.Mupload(var successful: boolean);
...
var
  ...
  begin
	...
	repeat
	  waiting(100);		{wait 0.1sec}
	  inc(loop);
	until not cts or inready or (loop >= loopend) or abort;
  ...
  Function recm:byte;  (* CRC を計算しない *)
	...
	begin
	  time:=gettickcount;
	  while cts and (gettickcount-time<1000) and not inready do
		;//application.processmessages;
...
Except
	  rslt:=9;
          success:=False;
End;
	  if (rslt=0) and cts then begin
 //		flowctrl( false );
		wk:=9;  (*10回待ちます *)
		waiting(6000); (*6秒間*)
		xmitchar(chr(INI));
		while cts and (detecthb(10)=false) and (wk>0) and (rslt=0) do begin
		  xmitchar(chr(INI));
		  dec(wk);
		end;
		if wk=0 then rslt:=9;
		if cts and (rslt=0) then begin
		  ...
		  repeat	  (* ファイル名を飛ばす *)
                	inch:=recm;
                        temp:=temp+chr(inch);
		  until not cts or (inch=$20) or (inch=EXT) or (inch=CAN) or error or abort;

                  MCS.GetFileName := Copy(temp,1,length(temp)-1);

		  if (inch=EXT) or (inch=CAN) or error or not cts or abort then rslt:=9
		  else begin
			temp:='';
			dumy:=0;
			repeat	(* ファイルサイズを得る *)
			  dumy:=recm;
			  temp:=temp+chr(dumy);
			until not cts or (dumy=$20) or error or abort;
			temp:=Copy(temp,1,length(temp)-1);
			filebytes:=StrToIntDef(Temp,0);
                        MCS.FileSize:=FileBytes;
			if filebytes=0 then rslt:=9
			else begin
			  temp:='';
			  repeat	(* IB の終わりまで *)
				dumy:=recm;
			  until not cts or (dumy=$0D) or error ;
			end;
...
Procedure TMyoComm.storebuff(var buffernum: byte; var paused, aborted: boolean);
  var loop: byte;
	  rslt: integer;
  begin
	loop := 0;
...
Function TMyoComm.Ztimedin:boolean;
  ...
  begin
    ...
    while (not inready) and cts and ((gettickcount-begcnt)<10000) and not(Abort) do
	  ;//application.processmessages;
	Ztimedin:=inready;
  end;
...
Function TMyoComm.Zfromunixdate(s:string):TDateTime;
  var
	dt : TDateTime;
...
Procedure TMyoComm.Zmakepackedheader(htype:byte;datas:longint;var hdr:headertype);
  begin
	hdr[0]:=htype;
	hdr[1]:=byte(datas);
...
Function TMyoComm.Zmakeunpackedheader(var hdr:headertype):longint;
  var
	l:longint;
  begin
	l:=hdr[4];
...
Function TMyoComm.ZcheckfileCRC(crcreq:longint):longint;
  var
    dumy:byte;
    loop:word;
    fbuf:filbuffer;
...
Procedure TMyoComm.Zsendstring(p:string);
  var
	loop:word;
  begin
...
Procedure TMyoComm.Zsendhex(b:byte);
  const
...
Procedure TMyoComm.Zsendhexheader(header:headertype);
  var
	loop:byte;
	bkup:crctype;
  begin
...
Function TMyoComm.ZcatchZDL:word;	(*エラーのとき_ZR_ERRを返す by Seishi*)
  var
	temp:byte;
  begin
...
Function TMyoComm.Zgethex(var err : boolean) : byte;
  var
    temp, wk : byte;
  begin
...
Procedure TMyoComm.Zgethexheader(var header:headertype;var err:boolean);
  var
	loop:byte;
	temp:byte;
	dumy:char;
...
Procedure TMyoComm.Zgetbinheader(var header:headertype;var err:boolean);
  var
	loop:word;
	temp:word;
	dumy:byte;
...
Function TMyoComm.Zgetheader(var header:headertype; var num:longint; var err :boolean):byte;
(* 判断がいまいちいい加減 :-) *)
  var
    temp : byte;
    pass : boolean;
...
Procedure TMyoComm.Zsendbyte(b:byte);
  begin
...
Procedure TMyoComm.Zsendblkbyte(var b; cnt:word);		{ｲﾝﾌﾟﾘﾒﾝﾄ by Seishi		 }
...
Procedure TMyoComm.Zsendbinheader(header:headertype);
  var
	loop:word;
	wk:byte;
  begin
...
Procedure TMyoComm.Zupload(var successful:boolean);
...
crcfoo:
  ...
  Procedure Zendrecv;
	...
	begin
	  ...
	  until (loop=0) or not cts;   (* ZFIN で終了出来なくてもよしとした *)
	end;
		  ...
		  case temp[0] of
			_FILE : begin
					...
					while filebuf_[pt]<>0 do
                                         Begin
                                           MCS.GetFileName:=MCS.GetFileName+char(FileBuf_[pt]);
                                           inc(pt);
                                         End;
					inc(pt);
                                        S_myo:='';
					while filebuf_[pt]>$20 do
                                         Begin
                                           ...
                                           inc(pt);
                                         End;
                                        MCS.FileSize := StrToIntDef(S_Myo,0);
					inc(pt);
					while (filebuf_[pt]>=ord('0'))
					  and (filebuf_[pt]<=ord('9')) do begin
					  orgftime := orgftime*8 + filebuf_[pt] - ord('0');
...
{			_FREECNT : begin  (* こんなの本当に呼び出されるのかなあ *)
  ...
  begin { of Zupload }
	...
	if success then begin
	  ...
	  until (recurs=_COMPL) or (retry=0) or error or not cts;
	  if not error then Zendrecv else Zcancel;
	end
	else error:=true;
	successful := not error and fileexists(MCS.FileName) and cts;
...
Procedure TMyoComm.Zdownload(var successful:boolean);
  (* The Resource 版からインプリメント by Seishi	*)
...
//	  if hoststat = 0 then dispstatus(cn);
	  ...
	  if not err then repeat
		...
		if not err then repeat
		  {$I-} blockreadm(filebuf_[0],8); {$I+}
                  MCS.TranSize:=MCS.TranSize+8*128;
		...
		until (frame<>_CRCG) or inready or err or not cts or Abort;
  ...
  Function Zsendfile(var err:boolean):byte;
	...
	begin
	  ...
	  until ((temp[0]<>_RINIT) and (temp[0]<>_CRC)) or err or not cts or abort;
	end;
  ...
  Procedure setZheader;
	...
	begin
	  clearbuff;
	  tmp := MCS.SendFileName + #$00;
	  sdek:=IntToStr(Get_File_Size);	{byte単位でﾌｧﾙｻｲｽﾞ}
	  tmp := tmp + sdek + ' ' + time1970(FileDateToDateTime(fileage(MCS.FileName)));
	  tmp := AnsiLowerCase(tmp);		{小文字化(MS漢字はｽﾙｰ)}
	  Move(tmp[1], filebuf_[0], length(tmp));
	end;
...
//	flowctrl(false);
	...
	successful := true;
	  if FileExists(MCS.FileName) then begin
		...
		else begin	(* ﾀｲﾑｵｰﾊﾞｰ（ZMODEM の起動が遅かった）又は ｷｬﾝｾﾙ *)
		  successful:=false;
		end;
	    closeFile(F_);
	  end
...
Procedure TMyoComm.waiting(tcount: integer);
(*tcountだけwaitする。n秒なら waiting(n*seccnt) *)
  var begcnt : integer;
  begin
	begcnt := gettickcount;
	while (gettickcount-begcnt < tcount) and Not(Abort) do
                 ;//Application.ProcessMessages;
  end;


Procedure TMyoComm.xmitchar(ch: char);
(*シリアル出力ポートがレディならchを出力する（ローカルモードなら出力しない）
  not cts なら何もせずに戻る *)
  var sttm : integer;
  begin
    if cts  then write(ch,1);

  end;



Function TMyoComm.receiveblock(var rpt; len:integer) : word;
var
 recvlen:integer;
 i      : integer;
 p:pchar;
begin
  ...
  if (wp<>rp) and (Len>0)  then
     begin
       For i:=0 to len-1 do
        begin
         p[i]:=File_Recv_Buf[rp];
         inc(rp);
         if Rp>49999 then Rp:=0;
         if wp=rp then
          begin
            result:=i+1;
            exit;
...
Procedure TMyoComm.stringout(s:String);
Begin
      SendString(s);
...
Function TMyoComm.RecvChar: Char;
begin

      if wp=rp then
         While Rp=Wp do ;//Application.ProcessMessages;

      result:=File_Recv_Buf[rp];
      inc(rp);
      if rp>49999 then
        rp:=0;
      InReady:=(wp<>rp);
...
Function TMyoComm.Get_File_Size : integer;
Var
    Fs: File of Byte;
begin
    if Fileexists(MCS.FileName) then Begin
       AssignFile(Fs,MCS.FileName);
       Reset(Fs);
       Result := FileSize(Fs);
       CloseFile(Fs);
     end
...
Procedure TMyoComm.blockreadM(var filebrock; n:word);
Begin
  BlockRead(F_,FileBrock,n*128,RecCount)
End;

Procedure TMyoComm.blockreadM2(var filebrock; n:word);
Begin
  BlockRead(f_,FileBrock,n,reccount);
End;

Procedure TMyoComm.flush;
begin
	rp:=wp;
        Inready:=False;
end;

Procedure TMyoComm.seekM2(n:longint; var rslt : boolean);
  begin
